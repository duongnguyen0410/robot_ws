...
controller_interface::CallbackReturn DiffDriveController::on_init()
{
  ...
  auto node = get_node();
  computed_vel_publisher_ = node->create_publisher<std_msgs::msg::Float64MultiArray>("computed_vel", 10);
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  const double velocity_left =
    (linear_command - angular_command * wheel_separation / 2.0) / left_wheel_radius * 3;
  const double velocity_right =
    (linear_command + angular_command * wheel_separation / 2.0) / right_wheel_radius * 3;

  // Publish velocity_left & velocity_right
  std_msgs::msg::Float64MultiArray computed_vel_msg;
  computed_vel_msg.data = {velocity_left, velocity_right};
  computed_vel_publisher_->publish(computed_vel_msg);
