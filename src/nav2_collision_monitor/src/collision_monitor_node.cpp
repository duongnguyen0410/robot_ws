...
CollisionMonitor::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  std::string cmd_vel_test_topic;

  // Obtaining ROS parameters
  if (!getParameters(cmd_vel_in_topic, cmd_vel_out_topic, cmd_vel_test_topic)) {
  ...
  cmd_vel_out_pub_ = this->create_publisher<geometry_msgs::msg::Twist>(
    cmd_vel_out_topic, 1);
  cmd_vel_test_sub_ = this->create_subscription<std_msgs::msg::Int32>(
    cmd_vel_test_topic, 1,
    std::bind(&CollisionMonitor::cmdVelTestCallBack, this, std::placeholders::_1));
...
void CollisionMonitor::cmdVelTestCallBack(std_msgs::msg::Int32::SharedPtr msg)
{
    RCLCPP_INFO(this->get_logger(), "Received command value: '%d'", msg->data);
    cmd_test_value_ = msg->data;
}
  ...
  std::string & cmd_vel_test_topic)
{
  ...
  nav2_util::declare_parameter_if_not_declared(
    node, "cmd_vel_test_topic", rclcpp::ParameterValue("int_topic"));
  cmd_vel_test_topic = get_parameter("cmd_vel_test_topic").as_string();
...
bool CollisionMonitor::processStop(
  Action & robot_action
)
{
  if (cmd_test_value_ == 1){
