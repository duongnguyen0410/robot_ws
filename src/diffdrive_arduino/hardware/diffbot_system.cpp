  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  try
  {
    ...
    cfg_.device_ip = info_.hardware_parameters["device_ip"];
    cfg_.port = std::stoi(info_.hardware_parameters["port"]);
    cfg_.timeout_ms = std::stoi(info_.hardware_parameters["timeout_ms"]);
    cfg_.enc_counts_per_rev = std::stoi(info_.hardware_parameters["enc_counts_per_rev"]);
    
    wheel_l_.setup(cfg_.left_wheel_name, cfg_.enc_counts_per_rev);
    wheel_r_.setup(cfg_.right_wheel_name, cfg_.enc_counts_per_rev);/* code */
  }
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  comms_.connect(cfg_.device_ip, cfg_.port);

  RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Successfully configured tibot!");
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  // std::cerr << "wheel_l_.cmd: " << wheel_l_.cmd << " wheel_r.cmd: " << wheel_r_.cmd << std::endl;

  // std::cerr << "wheel_l_.cmd: " << wheel_l_.cmd << " wheel_l.rads_per_count: " << wheel_l_.rads_per_count << " cfg_.loop_rate: " << cfg_.loop_rate << std::endl;

  // std::cerr << "wheel_r_.cmd: " << wheel_r_.cmd << " wheel_r.rads_per_count: " << wheel_r_.rads_per_count << " cfg_.loop_rate: " << cfg_.loop_rate << std::endl;
